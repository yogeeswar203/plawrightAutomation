  ...
  test.skip("TC 001: Validating the dynamic tabel", async ({ page }) => {
    ...
    for (const d1 of no_rows) {
      ...
      if (process_name === "Chrome") {
        ...
        console.log(
          `Chrome CPU percentage is: ${cpuload} and expected CPU Load is ${exp_cpuload}`,
        );
        break;
      }
    }
  ...
  test.skip("TC 002: Verify the Disk Space", async ({ page }) => {
    ...
    for (const data of allrows) {
      ...
      if (process_name === "Firefox") {
        ...
        console.log(
          `Chrome CPU percentage is: ${act_memory} and expected CPU Load is ${exp_memory}`,
        );
        break;
      }
    }
 ...
 */
    ...
    console.log(
      `The no of row present ${row_count} and colums present ${col_count}`,
    );
